...
def checkBounds(w:World, loc:Location) -> bool:
    return (0 <= loc.ypos < w.ground.height) and (0 <= loc.xpos < w.ground.width)
...
'''
Prints the altitude of each cell in the console and 'A' for Agent, 'X' for Object or '.' for common wing
'''
...
def isValidMove(w:World, ag:Agent, loc:Location, visited):
    if checkBounds(w, loc) and not visited[loc.xpos][loc.ypos]:
